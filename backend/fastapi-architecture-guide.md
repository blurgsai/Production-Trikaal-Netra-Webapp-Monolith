...
├── features
│   ├── posts
│   │   ├── clients/        <- External API client + raw schemas
│   │   ├── models/         <- Domain models + mappers (anti-corruption layer)
...
│   ├── dependencies/       <- FastAPI dependency injection (HTTP client, DB, etc.)
...
In typical FastAPI codebases:
- Raw database/API response schemas are reused directly as endpoint response models
- When an external API or database schema changes, you modify route handlers, services, and tests across the repo
- Features become tangled – changing one area breaks another
- No clear boundary between "external data shapes" and "your app's data shapes"

**Goal:** Structure code so that any single type of change (external schema, business logic, endpoint design) stays local – touching only the files responsible for that concern.

---

## 2. Architecture Overview

We use **feature-sliced architecture** with an **anti-corruption layer**, adapted for FastAPI.

### Key Principles

1. Code is organized by **feature**, not by technical role (no top-level `/models`, `/routes`, `/schemas` folders)
2. Each feature is **self-contained** – it owns its external clients, domain logic, and endpoints
3. Routers **never see raw external schemas** – they only know about domain models
4. Services **orchestrate** – they call clients, map through the anti-corruption layer, and return domain models
5. The mapper **absorbs schema changes** so nothing downstream needs to move
...
```
Layer 1: Client  ->  "Talk to external services / database"
Layer 2: Model   ->  "Translate external shapes -> our shapes"
Layer 3: Service ->  "Business logic, orchestration, error handling"
Layer 4: Router  ->  "HTTP endpoints (only sees domain models)"
```

Data flows in one direction:

```
Client  ->  Model (mapper)  ->  Service  ->  Router
```

### Who Knows What

| Layer | Knows about external schema? | Knows about domain model? | Knows about FastAPI? |
|-------|------------------------------|---------------------------|----------------------|
| `clients/` | Yes | No | No |
| `models/` (mapper) | Yes | Yes | No |
| `services/` | No (calls mapper) | Yes | No |
| `router/` | No | Yes | Yes |
...
### Layer 1 – Client (`features/users/clients/`)

**Job:** Define the raw external schemas and fetch data. Knows nothing about FastAPI.

```python
# clients/__init__.py -- mirrors the external API exactly
...
## 5. The Anti-Corruption Layer in Action
...
```
router/  ->  services/  ->  models/ + clients/  ->  shared/
```
...
## 9. Quick Reference – Where Does Each Change Go?
...
## 10. Comparison: React vs FastAPI Layers
...
The same architectural principles apply to both frontend and backend. Schema changes stay local. Features are self-contained. Layers have one job.
