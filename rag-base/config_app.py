...
def mongo_uri_input_section():
    st.header("⛁ Mongo URI")
    st.markdown("Enter MongoDB URI")

    with st.expander("Load your mongo uri"):
        ...
        if st.button("✅ Load MongoDB URI", use_container_width=True):
            if mongo_uri:
                try:
                    st.session_state.mongo_uri = mongo_uri
                    st.success("✅ MongoDB URI loaded into session state!")
                    logger.info(f"Loaded MongoDB URI: {mongo_uri}")
                except Exception as e:
                    logger.error(f"Error loading mongo uri: {str(e)}")
                    st.error(f"Error loading mongo uri: {str(e)}")
                
    return 


def main():
    # Render Mongo URI input section first
    mongo_uri_input_section()
    ...
    if st.button("🗂️ Process & Ingest configurations into database",width="stretch"):
        try:
            client = MongoClient(st.session_state.mongo_uri)
            db = client[os.getenv("MONGO_DB_NAME", "dev")]
            collection_settings = db["copilot_settings"]
            collection_sources = db["copilot_sources"]

            # ==========================================
            # 1. Upsert the Settings Document
            # ==========================================
            settings_doc = {
                "selected_llm": {
                    "provider": st.session_state.selected_provider,
            ...
            # Use _id: "default" as the query filter to always update the main config
            collection_settings.update_one(
                {"_id": "default"},
                {"$set": settings_doc},
                upsert=True
            )
            st.success("✅ `copilot_settings` updated successfully!")

            # ==========================================
            # 2. Upsert the Source Documents
            # ==========================================
            bulk_operations = []
            enabled_sources = st.session_state.selected_sources
            
            # Iterate over all sources currently loaded in the session state
            for source_type, config_data in st.session_state.source_configs.items():
                if not config_data:
                    # Skip sources that were added to the UI but not actually configured yet
                    continue
                
                # Determine enabled status based on the selected_sources list
                is_enabled = source_type in enabled_sources
                
                # Map tools (handling both "tools" or "selected_tools" keys depending on your config shape)
                tools = config_data.get("tools", config_data.get("selected_tools", []))
                
                # Map schema resources into an array format as required by your MongoDB schema
                resources = config_data.get("resources", [])
                if not resources and "schema_resource" in config_data:
                    # Convert a flat schema_resource dict into the target array format
                    schema = config_data["schema_resource"]
                    resources = [{
                        "type": "schema",
                        "name": schema.get("name", f"{source_type}_schema"),
                        "description": schema.get("description", ""),
                        "content": schema.get("content", "")
                    }]

                # Construct the final source document
                source_doc = {
                ...
                # Create an upsert operation matching on "source_type"
                # This prevents duplicate documents for the same source type
            ...
            # Execute the bulk operation for sources
            if bulk_operations:
                result = collection_sources.bulk_write(bulk_operations)
                total_processed = result.upserted_count + result.modified_count
                st.success(f"✅ `copilot_sources`: {total_processed} configurations ingested/updated!")
            else:
                st.info("No valid source configurations found to ingest.")
