...
@pytest.mark.asyncio
async def test_mongo_schema_resource(mcp_server):
    """
    Test that the MCP server correctly serves the mongo_schema resource
    ...
    # 2. Extract the expected mongo_schema content from YAML
    with open(config_path, "r") as f:
        config_data = yaml.safe_load(f)
        
    assert "mongo" in config_data, "mongo configuration missing in YAML"
    assert "schema_resource" in config_data["mongo"], "schema_resource missing in mongo config"
    
    expected_content = config_data["mongo"]["schema_resource"]["content"]
    ...
    try:
        ...
        assert "mongo_schema" in context, "mongo_schema resource not found in MCP server"
        
        # 6. Verify the content contains the expected schema
        # Note: MongoRAGSource prepends `tool_set_description` and `schema_name` to the content
        actual_content = context["mongo_schema"]["content"]
