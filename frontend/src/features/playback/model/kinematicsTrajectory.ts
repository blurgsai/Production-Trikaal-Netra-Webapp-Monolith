...
// Shared across the kinematics family (sudden_stop, anomalous_acceleration,
// anomalous_jerk). Registered under each type's key in trajectoryOverrideRegistry.ts.
// These events are instantaneous or sub-minute (sudden_stop has duration 0), which is
// shorter than the AIS sample gap — so we enforce a minimum visible span centred on the
// event, otherwise the highlighted segment would collapse to nothing on the track.
// (Flattened from model/shared/kinematicsTrajectory.ts to satisfy the flat-layer CI rule.)
...
export function getKinematicsTrajectoryOverrides(
