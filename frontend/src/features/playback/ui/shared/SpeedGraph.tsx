...
interface SpeedGraphProps {
  ...
  rangeStartMs: number;
  rangeEndMs: number;
  // When true, the alert zone (red fill) is above the threshold instead of below it.
  inverted?: boolean;
}
...
export function SpeedGraph({
  ...
  // Computed before the empty-data early return so hook order stays stable across renders.
    ...
    >
      ...
      >
        <g transform={`translate(${pad.left}, ${pad.top})`}>
          ...
          {hasThreshold && (
            <>
              <path d={buildAreaPath(!inverted)} fill="rgba(255,68,68,0.15)" />
              <path d={buildAreaPath(inverted)}  fill="rgba(68,255,68,0.15)" />
            </>
          )}
