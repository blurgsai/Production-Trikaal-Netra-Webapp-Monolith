...
import { useThreatDetail } from "../useThreatDetail";
import type {
...
describe("useThreatDetail", () => {
  ...
  describe("initial state", () => {
    ...
    it("returns isLoading=false when eventId is undefined (query disabled)", () => {
      vi.mocked(getEventDetail).mockReturnValue(new Promise(() => {}));
      const { result } = renderHook(() => useThreatDetail(undefined), {
        wrapper: createWrapper(),
      });
    ...
    it("does not call getEventDetail when eventId is undefined", () => {
      ...
      renderHook(() => useThreatDetail(undefined), { wrapper: createWrapper() });
      expect(getEventDetail).not.toHaveBeenCalled();
    });

    it("does not call getEventDetail when eventId is empty string", () => {
      vi.mocked(getEventDetail).mockResolvedValue(mockEventDetail);
      renderHook(() => useThreatDetail(""), { wrapper: createWrapper() });
      expect(getEventDetail).not.toHaveBeenCalled();
    });

    it("calls getEventDetail when eventId is provided", async () => {
      vi.mocked(getEventDetail).mockResolvedValue(mockEventDetail);
      renderHook(() => useThreatDetail("evt-001"), { wrapper: createWrapper() });
  ...
  describe("success state", () => {
    ...
    it("maps structuredFields label", async () => {
      vi.mocked(getEventDetail).mockResolvedValue(mockEventDetail);
      const { result } = renderHook(() => useThreatDetail("evt-001"), {
        wrapper: createWrapper(),
      });
      await waitFor(() => expect(result.current.data?.structuredFields?.[0].label).toBe("Vessels Involved"));
    });

    it("maps structuredFields value", async () => {
      vi.mocked(getEventDetail).mockResolvedValue(mockEventDetail);
      const { result } = renderHook(() => useThreatDetail("evt-001"), {
        wrapper: createWrapper(),
      });
      await waitFor(() => expect(result.current.data?.structuredFields?.[0].value).toBe("3"));
    });
    ...
    it("maps linkedArticlePreview title", async () => {
      vi.mocked(getEventDetail).mockResolvedValue(mockEventDetail);
      const { result } = renderHook(() => useThreatDetail("evt-001"), {
        wrapper: createWrapper(),
      });
      await waitFor(() => expect(result.current.data?.linkedArticlePreview?.title).toBe("Red Sea Tensions Rise"));
    });
  ...
  describe("edge cases", () => {
    it("handles event detail without primary_location", async () => {
      vi.mocked(getEventDetail).mockResolvedValue({ ...mockEventDetail, primary_location: undefined });
      const { result } = renderHook(() => useThreatDetail("evt-001"), { wrapper: createWrapper() });
    ...
    it("handles event detail without locations array", async () => {
      vi.mocked(getEventDetail).mockResolvedValue({ ...mockEventDetail, locations: undefined as unknown as WorldMonitorLocationApiResponse[] });
      const { result } = renderHook(() => useThreatDetail("evt-001"), { wrapper: createWrapper() });
      await waitFor(() => expect(result.current.data?.locations).toBeUndefined());
    });

    it("handles event detail without structured_fields", async () => {
      vi.mocked(getEventDetail).mockResolvedValue({ ...mockEventDetail, structured_fields: undefined as unknown as StructuredFieldApiResponse[] });
      const { result } = renderHook(() => useThreatDetail("evt-001"), { wrapper: createWrapper() });
    ...
    it("handles event detail without linked_article_preview", async () => {
      vi.mocked(getEventDetail).mockResolvedValue({ ...mockEventDetail, linked_article_preview: undefined });
      const { result } = renderHook(() => useThreatDetail("evt-001"), { wrapper: createWrapper() });
    ...
    it("handles event detail without reasoning", async () => {
      vi.mocked(getEventDetail).mockResolvedValue({ ...mockEventDetail, reasoning: undefined });
      const { result } = renderHook(() => useThreatDetail("evt-001"), { wrapper: createWrapper() });
      await waitFor(() => expect(result.current.data?.reasoning).toBeUndefined());
    });

    it("handles event detail without relevance_score", async () => {
      vi.mocked(getEventDetail).mockResolvedValue({ ...mockEventDetail, relevance_score: undefined });
      const { result } = renderHook(() => useThreatDetail("evt-001"), { wrapper: createWrapper() });
      await waitFor(() => expect(result.current.data?.relevanceScore).toBeUndefined());
    });

    it("handles event detail without enriched_at", async () => {
      vi.mocked(getEventDetail).mockResolvedValue({ ...mockEventDetail, enriched_at: undefined });
      const { result } = renderHook(() => useThreatDetail("evt-001"), { wrapper: createWrapper() });
      await waitFor(() => expect(result.current.data?.enrichedAt).toBeUndefined());
    });

    it("handles empty locations array", async () => {
      vi.mocked(getEventDetail).mockResolvedValue({ ...mockEventDetail, locations: [] });
      const { result } = renderHook(() => useThreatDetail("evt-001"), { wrapper: createWrapper() });
      await waitFor(() => expect(result.current.data?.locations).toEqual([]));
    });

    it("handles empty structured_fields array", async () => {
      vi.mocked(getEventDetail).mockResolvedValue({ ...mockEventDetail, structured_fields: [] });
      const { result } = renderHook(() => useThreatDetail("evt-001"), { wrapper: createWrapper() });
    ...
    it("handles relevance_score of 0", async () => {
      vi.mocked(getEventDetail).mockResolvedValue({ ...mockEventDetail, relevance_score: 0 });
      const { result } = renderHook(() => useThreatDetail("evt-001"), { wrapper: createWrapper() });
      await waitFor(() => expect(result.current.data?.relevanceScore).toBe(0));
    });

    it("handles relevance_score of 1.0", async () => {
      vi.mocked(getEventDetail).mockResolvedValue({ ...mockEventDetail, relevance_score: 1.0 });
      const { result } = renderHook(() => useThreatDetail("evt-001"), { wrapper: createWrapper() });
    ...
    it("handles linked_article_preview with tags", async () => {
      vi.mocked(getEventDetail).mockResolvedValue({
        ...mockEventDetail,
        linked_article_preview: { ...mockEventDetail.linked_article_preview!, tags: ["conflict", "maritime"] },
      });
      const { result } = renderHook(() => useThreatDetail("evt-001"), { wrapper: createWrapper() });
    ...
    it("handles linked_article_preview with locations", async () => {
      vi.mocked(getEventDetail).mockResolvedValue({
        ...mockEventDetail,
        linked_article_preview: { ...mockEventDetail.linked_article_preview!, locations: [{ name: "Red Sea", lat: 0, lng: 0, role: "primary" }] },
      });
      const { result } = renderHook(() => useThreatDetail("evt-001"), { wrapper: createWrapper() });
  ...
  describe("enabled behavior", () => {
    ...
    it("calls getEventDetail with the provided eventId", async () => {
      vi.mocked(getEventDetail).mockResolvedValue(mockEventDetail);
      renderHook(() => useThreatDetail("evt-999"), { wrapper: createWrapper() });
  ...
  describe("cache and refetch", () => {
    it("does not refetch on re-render with same eventId", async () => {
      vi.mocked(getEventDetail).mockResolvedValue(mockEventDetail);
      const { rerender } = renderHook(() => useThreatDetail("evt-001"), { wrapper: createWrapper() });
      await waitFor(() => expect(getEventDetail).toHaveBeenCalledTimes(1));
      rerender();
