...
// Disables the vessel marker CSS transition during pan/zoom to prevent drift
function MarkerTransitionGuard({
  ...
  useEffect(() => {
    const getIcon = () => (markerRef.current as any)?._icon as HTMLElement | undefined
...
// Fits the map to the full trajectory bounds when a new vessel loads
function FitBounds({
  fullTrajectory,
  fitKey,
}: {
...
// Renders a single event marker. location.lat/lon are already swapped by the mapper.
function EventMarker({
  ...
  const color = SEVERITY_COLOR[event.severity ?? ''] ?? '#ff9800'
  ...
  return (
    ...
    >
      <Popup>
        ...
        <button
          style={{ marginTop: 6, cursor: 'pointer', color: '#1976d2', background: 'none', border: 'none', padding: 0, fontSize: 13 }}
...
export const FocusModeMap = ({
  ...
  // Apply CSS transition after each render so it catches the marker first appearing
  // and updates duration when playbackSpeed changes
  useEffect(() => {
    const icon = (markerRef.current as any)?._icon as HTMLElement | undefined
    if (!icon) return
    const seconds = Math.max(0.15, 1 / playbackSpeed)
    icon.style.transition = `transform ${seconds}s linear`
  })

  // Rotate the arrow to face the vessel heading
  useEffect(() => {
    const icon = (markerRef.current as any)?._icon as HTMLElement | undefined
