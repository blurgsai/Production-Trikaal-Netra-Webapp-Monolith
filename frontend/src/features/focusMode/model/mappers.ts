...
export const mapVessel = (raw: VesselRaw): Vessel => ({
  // String() here prevents IEEE 754 precision loss when the backend later
  // receives this value in a URL path — the backend must also return vessel_id
  // as a JSON string ("vessel_id": "...") to avoid corruption at JSON.parse time.
...
export const mapEvent = (raw: EventRaw): FocusEvent => {
  ...
  // GeoJSON is [lon, lat] — swap to our domain { lat, lon }
